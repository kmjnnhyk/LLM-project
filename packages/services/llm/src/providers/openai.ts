 ...
 * @returns 영양제 추천 결과
 */
export async function getNutritionRecommendationWithOpenAI(
  data: OnboardingData,
  ...
  if (unsupportedModels.includes(model)) {
    ...
    if (isDev) {
      console.warn(
        `⚠️  모델 ${model}은 Structured Outputs를 지원하지 않습니다. gpt-4o로 자동 변경합니다.`
      );
    }
    model = 'gpt-4o';
  }
  ...
  try {
      ...
      messages: [
        {
          role: 'system',
          content:
            '당신은 영양제 추천 전문가입니다. 사용자 정보를 분석하여 맞춤형 영양제를 추천합니다.',
        },
        {
          role: 'user',
          content: prompt,
    ...
    if (isDev) {
      ...
      if (error && typeof error === 'object') {
        ...
        console.error('status 속성:', (error as { status?: unknown }).status);
        console.error('code 속성:', (error as { code?: unknown }).code);
        console.error('type 속성:', (error as { type?: unknown }).type);
      }
      console.error('==========================');
    }
    ...
    if (error && typeof error === 'object') {
      const apiError = error as { status?: number; message?: string; code?: string; type?: string };
