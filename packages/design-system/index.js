...
function getComponentMetadata(componentName) {
  try {
    ...
    console.error(`Error reading metadata for ${componentName}: ${error.message}`);
...
function getSelectedComponentsDocs(componentNames) {
  const docsObject = {};
  console.log(`✅ Getting documentation for components: ${componentNames.join(', ')}`);
...
server.tool(
  ...
  {
    selectedComponents: z.array(z.string()).describe('The names of the components'),
  },
  (input) => {
    console.log(`✅ Selected components: ${input.selectedComponents.join(', ')}`);
...
server.tool(
  ...
  {
    component_names: z.array(z.string()).describe('The names of the components'),
  },
